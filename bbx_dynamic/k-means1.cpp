...
double KMeans::CalcDistance(const double* x, const double* u, int dimNum)
{
	...
	*/
	/**/
	double temp = 0;
	double BOARD_WEIGHT = 0.9;															//上车距离的权重
																						//实现距离等于	= a * d上	+ （1 - a） * d下
	for (int d = 0; d < dimNum; d++)
	{
		if (d >= 2)
			BOARD_WEIGHT = 1 - BOARD_WEIGHT;
		temp += BOARD_WEIGHT * (x[d] - u[d]) * (x[d] - u[d]);
	}
	/*
	string ss = to_string(x[1]);
	ss += ",";
	ss += x[0];

	string desc = to_string(u[1]);
	desc +=  ",";
	desc += u[0];

	double dd = atof(test(ss, desc).c_str());*/
